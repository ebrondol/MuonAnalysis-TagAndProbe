...
double yminGraph(TGraphAsymmErrors *g) {
    double ret = 1;
    if (g != 0) {
        int n = g->GetN();
        for (int i = 0; i < n; ++i) {
            ret = TMath::Min(ret, g->GetY()[i] - g->GetErrorYlow(i));
        }
    }
    return ret;
}
...
void doRatio(TGraphAsymmErrors *hfit, TGraphAsymmErrors *href, TString alias, const char *xtitle) {
    ...
    ratio.Draw("P SAME");
    if (autoScale) {    
        frame->GetYaxis()->SetRangeUser(1+1.5*min,1+1.2*max);
    } else {
    ...
    if (fOut) { fOut->WriteTObject(&ratio,"ratio_"+alias, "Overwrite");  }
    delete frame;
}
...
void refstackNamed(TDirectory *fit, TDirectory *refd, TString alias, TString fitname, TString refname) {
    if (fit == 0) {
    ...
    TCanvas *pref = (TCanvas*) getFromPrefix(refd->GetDirectory("fit_eff_plots"), refname);
    if (pref == 0) {
        std::cerr << "NOT FOUND: " << "fit_eff_plots/"+refname << " in " << refd->GetName() << std::endl;
        return;
    }
    ...
    if (yMin < yMax) {
        ...
        double myYMin = TMath::Min(yminGraph(hfit),yminGraph(href));
        myYMin = TMath::Max(0.0, 1.0 - 1.45*(1.0-myYMin));
        setRangeY(pref, myYMin, yMax);
    }
...
void refstack(TDirectory *fit, TDirectory *refd,  TString alias, TString fitname) {
    refstackNamed(fit,refd,alias,fitname,fitname);
}
