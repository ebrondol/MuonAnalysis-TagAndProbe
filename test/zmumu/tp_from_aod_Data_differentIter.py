...
if DivideEarly: 
    ...
    process.pCutTracksEarly = process.pCutTracks.clone(src = 'tracksNoMuonSeeded')
    process.tkTracksEarly = process.tkTracks.clone(src = 'pCutTracksEarly')
    process.tkTracksNoZEarly = process.tkTracksNoZ.clone(src = 'tkTracksEarly')
    process.preTkMatchSequenceZ.replace(
            process.tkTracksNoZ, process.tkTracksNoZ +
            process.tracksNoMuonSeeded + process.pCutTracksEarly + process.tkTracksEarly + process.tkTracksNoZEarly)
    process.staToTkMatchEarly = process.staToTkMatch.clone(matched = 'tkTracksEarly')
    process.staToTkMatchNoZEarly = process.staToTkMatchNoZ.clone(matched = 'tkTracksNoZEarly')
    process.staToTkMatchSequenceZ.replace( process.staToTkMatch, process.staToTkMatch + process.staToTkMatchEarly )
    process.staToTkMatchSequenceZ.replace( process.staToTkMatchNoZ, process.staToTkMatchNoZ + process.staToTkMatchNoZEarly )
    process.tpTreeSta.variables.tkEarly_deltaR     = cms.InputTag("staToTkMatchEarly","deltaR")
    process.tpTreeSta.variables.tkEarly_deltaEta   = cms.InputTag("staToTkMatchEarly","deltaEta")
    process.tpTreeSta.variables.tkEarly_deltaR_NoZ   = cms.InputTag("staToTkMatchNoZEarly","deltaR")
    process.tpTreeSta.variables.tkEarly_deltaEta_NoZ = cms.InputTag("staToTkMatchNoZEarly","deltaEta")

if DivideIterations:
  iterList = ['initialStep', 'lowPtTripletStep', 'pixelPairStep', 'detachedTripletStep', 
	      'mixedTripletStep', 'pixelLessStep', 'tobTecStep', 'jetCoreRegionalStep', 
              'conversionStep', 'muonSeededStepInOut', 'muonSeededStepOutIn']
  for iterName in iterList:
    setattr(process, "tracksNoMuonSeeded"+iterName, cms.EDFilter("TrackSelector",
      src = cms.InputTag("generalTracks"),
      cut = cms.string("isAlgoInMask('%s')" % iterName )
      )
    )
    setattr(process, "pCutTracks"+iterName, process.pCutTracks.clone(src = cms.InputTag("tracksNoMuonSeeded%s" % iterName)))
    setattr(process, "tkTracks"+iterName, process.tkTracks.clone(src = cms.InputTag("pCutTracks%s" % iterName)))
    setattr(process, "tkTracksNoZ"+iterName, process.tkTracksNoZ.clone(src = cms.InputTag("tkTracks%s" % iterName)))
    setattr(process.RandomNumberGeneratorService, "tkTracksNoZ"+iterName, cms.PSet( initialSeed = cms.untracked.uint32(81) ))
    process.preTkMatchSequenceZ.replace(process.tkTracksNoZ, process.tkTracksNoZ + getattr(process, "tracksNoMuonSeeded%s" % iterName) 
            + getattr(process, "pCutTracks%s" % iterName) + getattr(process, "tkTracks%s" % iterName) + getattr(process, "tkTracksNoZ%s" % iterName))
    setattr(process, "staToTkMatch"+iterName, process.staToTkMatch.clone(src = cms.InputTag("tkTracks%s" % iterName)))
    setattr(process, "staToTkMatchNoZ"+iterName, process.staToTkMatchNoZ.clone(src = cms.InputTag("tkTracksNoZ%s" % iterName)))
    process.staToTkMatchSequenceZ.replace(process.staToTkMatch, process.staToTkMatch + getattr(process, "staToTkMatch%s" % iterName))
    process.staToTkMatchSequenceZ.replace(process.staToTkMatchNoZ, process.staToTkMatchNoZ + getattr(process, "staToTkMatchNoZ%s" % iterName))
    setattr(process.tpTreeSta.variables, "tk_"+iterName+"_deltaR", cms.InputTag("staToTkMatch%s" % iterName,"deltaR"))
    setattr(process.tpTreeSta.variables, "tk_"+iterName+"_deltaEta", cms.InputTag("staToTkMatch%s" % iterName,"deltaEta"))
    setattr(process.tpTreeSta.variables, "tk_"+iterName+"_deltaR_noZ", cms.InputTag("staToTkMatchNoZ%s" % iterName,"deltaR"))
    setattr(process.tpTreeSta.variables, "tk_"+iterName+"_deltaEta_noZ", cms.InputTag("staToTkMatchNoZ%s" % iterName,"deltaEta"))
...
if False: # turn on for tracking efficiency using L1 seeds
    ...
    process.l1ToTkMatchEarly    = process.staToTkMatchEarly.clone(src = "probeL1", srcTrack="none")
    process.l1ToTkMatchNoZEarly = process.staToTkMatchNoZEarly.clone(src = "probeL1", srcTrack="none")
    process.tpTreeL1 = process.tpTreeSta.clone(
        ...
        variables = cms.PSet(
            ...
            tkEarly_deltaR     = cms.InputTag("l1ToTkMatchEarly","deltaR"),
            tkEarly_deltaEta   = cms.InputTag("l1ToTkMatchEarly","deltaEta"),
            tkEarly_deltaR_NoZ   = cms.InputTag("l1ToTkMatchNoZEarly","deltaR"),
            tkEarly_deltaEta_NoZ = cms.InputTag("l1ToTkMatchNoZEarly","deltaEta"),
        ),
        flags = cms.PSet(
        ),
        tagVariables = cms.PSet(
    ...
    process.tagAndProbeTkL1 = cms.Path(
        ...
        process.l1ToTkMatchEarly + process.l1ToTkMatchNoZEarly +
        process.tpTreeL1
    )
...
process.RandomNumberGeneratorService.tkTracksNoZEarly = cms.PSet( initialSeed = cms.untracked.uint32(81) )
...
process.TFileService = cms.Service("TFileService", fileName = cms.string("tnpZ_Data_PromptReco_differentIter_prova.root"))
